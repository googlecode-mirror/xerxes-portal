...
{		
	...
	arrID.shift();  // remove first element
	var source = arrID.shift();  // second element is source
	var recordNumber = arrID.join('_'); // remainder is id
	...
	new Ajax.Request(url, {
		...
		"onSuccess": function (ajaxRequest) {
			...
			{
				numSavedRecords--;
				$$('#saveRecordOption_' + source + '_' + recordNumber + ' .temporary_login_note').each(function(node) {
					node.remove();
				});
				$('folder_' + source + '_' + recordNumber).src = "images/folder.gif";
				$(id).update( xerxes_labels['text_results_record_save_it'] );
				$(id).removeClassName("saved");
				
				// remove label input
				
				var label_input = $('label_' + source + '_' + recordNumber);
			...
			{				
				...
				numSavedRecords++;
				$('folder_' + source + "_" + recordNumber).src = "images/folder_on.gif";
				...
				{	
					...
					input_div.id = "label_" + source + "_" + recordNumber; 
