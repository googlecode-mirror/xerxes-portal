...
<xsl:include href="xerxes-utils.xsl"/>
<xsl:output method="xml" indent="yes" encoding="UTF-8"/>

<!--
	
Xerxes: included Context Object to MODS conversion; see xsl:include mods-utils.xsl above and 
	template calls to contextObject below 2006/08/28  dwalker
	
Xerxes: included format and peer review fields 2007/01/10  dwalker
...
-->
	...
	<xsl:template name="marcRecord">
		...
		<xsl:for-each select="marc:datafield[@tag=245]">
			<titleInfo>
				<xsl:variable name="title">
					<xsl:choose>
						<!-- Xerxes start -->
						<xsl:when test="marc:subfield[@code='a'] = 'Review: [untitled]' and ../marc:datafield[@tag='SID']/marc:subfield[@code='b'] = 'JSTOR_XML'">
							<xsl:for-each select="../marc:datafield[@tag=600]">
								<xsl:value-of select="marc:subfield[@code='a']" />
								<xsl:if test="following-sibling::marc:datafield[@tag=600]">
									<xsl:text>; </xsl:text>
								</xsl:if>
							</xsl:for-each>
						</xsl:when>
		...
		<genre authority="local">
			<xsl:call-template name="format">
				<xsl:with-param name="sid" select="marc:datafield[@tag='SID']/marc:subfield[@code='b']" />
			</xsl:call-template>
		</genre>
		<!-- end Xerxes -->
		...
		<originInfo>
			...
			<xsl:for-each select="marc:datafield[@tag=260]/marc:subfield[@code='a']">
				<place>
					<placeTerm>
						...
						<!-- Xerxes start -->
						<!-- changed this from the chopPunctuationFront template, since values not
						     coming thru; to chopPunctuation template 2007-01-13  -->
			...
			<xsl:if test="marc:datafield[@tag='YR ']/marc:subfield[@code='a'] != '^^^^'">
				<dateIssued>
					<xsl:value-of select="marc:datafield[@tag='YR ']/marc:subfield[@code='a']" />
				</dateIssued>
			</xsl:if>
		...
		<xsl:for-each select="marc:datafield[@tag=501 or @tag=504 or @tag=506 or @tag=507 or @tag=508 or  @tag=513 or @tag=514 or @tag=515 or @tag=516 or @tag=522 or @tag=524 or @tag=525 or @tag=526 or @tag=530 or @tag=533 or @tag=534 or @tag=535 or @tag=536 or @tag=538 or @tag=540 or @tag=541 or @tag=544 or @tag=545 or @tag=546 or @tag=547 or @tag=550 or @tag=552 or @tag=555 or @tag=556 or @tag=561 or @tag=562 or @tag=565 or @tag=567 or @tag=580 or @tag=581 or @tag=583 or @tag=584 or @tag=585 or @tag=586]">
			<note>
				<xsl:call-template name="uri"/>
				<xsl:variable name="str">
		...
		<!-- xerxes start -->
		<xsl:choose>
			<xsl:when test="contains(marc:datafield[@tag='SID']/marc:subfield[@code='b'],'EBSCO') and 
						  contains(marc:datafield[@tag=856]/marc:subfield[@code='i'],'T')">
				<xsl:variable name="acc_number" select="marc:datafield[@tag=016]/marc:subfield[@code='a']" />
				<xsl:variable name="db_code" select="substring(marc:datafield[@tag='SID']/marc:subfield[@code='b'],string-length(marc:datafield[@tag='SID']/marc:subfield[@code='b'])-2,3)" />	
				<location>
					<url>http://search.epnet.com/direct.asp?an=<xsl:value-of select="$acc_number" />&amp;db=<xsl:value-of select="$db_code" /></url>
				</location>
			</xsl:when>
			
			<xsl:when test="contains(marc:datafield[@tag='SID']/marc:subfield[@code='b'],'ERIC') and substring(marc:datafield[@tag='ERI']/marc:subfield[@code='a'],1,2) = 'ED'">
				<xsl:if test="number(substring(marc:datafield[@tag='ERI']/marc:subfield[@code='a'],3)) >= 340000">
					<location>
						<url>http://www.eric.ed.gov/ERICWebPortal/contentdelivery/servlet/ERICServlet?accno=<xsl:value-of select="marc:datafield[@tag='ERI']/marc:subfield[@code='a']" /></url>
					</location>
				</xsl:if>
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template name="relatedPart">
		<xsl:if test="@tag=773">
			...
			<!-- start Xerxes -->
			<part>
				<xsl:call-template name="contextObjectNumbers"/>
				<xsl:call-template name="contextObjectPages"/>
				<xsl:call-template name="contextObjectDate"/>
	...
	<xsl:template name="nameABCDQ">
		...
		<xsl:choose>
			<xsl:when test="contains($namePart,',')">
				<namePart type="family">
					<xsl:value-of select="normalize-space(substring-before($namePart,','))"/>
				</namePart>
				<namePart type="given">
					<xsl:value-of select="normalize-space(substring-after($namePart,','))"/>				
				</namePart>
			</xsl:when>
			<xsl:otherwise>
