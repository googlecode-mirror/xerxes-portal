	...
	{	
		...
		{			
			...
			{	
				...
				$configAuthenticationPage = $objRegistry->getConfig("AUTHENTICATION_PAGE", false);
				...
				if ( ! $configAuthenticationPage )
				{
					$configBaseURL = $objRequest->url_for(array("base" => "authenticate", "action" => "login"));
					$configAuthenticationPage = ''; 
				}
				...
				$bolShowServer = true; if ( $format == "xerxes-xml" ) $bolShowServer = false;
		...
		private function setHeader($format)
		{
			$arrFormats = array(
				...
				"bibliographic" => "Cache-Control: private; Content-type: application/x-research-info-systems",
				"embed_html_js" => "Content-type: application/javascript",
				"ris" => "Content-type: text/plain",
				"rss" => "Content-type: text/xml",
				"xerxes-xml" => "Content-type: text/xml",
				"text-file" => "Content-Disposition: attachment; Content-type: text/plain"
			);
