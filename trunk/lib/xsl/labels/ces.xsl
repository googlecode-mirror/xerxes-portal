...
<!--
 ...
 version: $Id: eng.xsl 1030 2010-01-05 18:42:09Z dwalker@calstate.edu $
 package: Xerxes
...
-->
	...
	<xsl:variable name="text_databases_az_databases">databáze</xsl:variable>
	...
	<xsl:variable name="text_header_my_collections">Mé kolekce</xsl:variable>
	...
	<xsl:variable name="text_header_savedrecords">Mé uložené záznamy</xsl:variable>
	...
	<xsl:template name="text_results_language">
		<xsl:if test="language and language != 'English' and format != 'Video'">
			<span class="resultsLanguage"> v jazyce <xsl:value-of select="language" /></span>
		</xsl:if>
	</xsl:template>
